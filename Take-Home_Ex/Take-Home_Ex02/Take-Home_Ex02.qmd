...
In this exercise, we will analyse the various data from various sources . There are 11 datasets used, as outlined in sections 3.1 and 3.2.
...
-   *Master Plan 2019 Subzone Boundary (No Sea*) (i.e. \`MPSZ-2019\`) in ESRI shapefile format was downloaded from [data.gov.sg](https://beta.data.gov.sg/collections/1749/view). This data provides the sub-zone boundary of URA Master Plan 2019.
...
-   `ggpubr` for elegant data visualisation

-   `knitr` for creating html table

-   `performance` is the utilities for computing measures to assess model quality
...
For the purpose of this exercise, we will only focus on the ***Weekday afternoon peak (5pm to 8pm)***, thus the code chunk below is used to extract the commuting flows on weekday and between 5 and 8 o'clock.
...
#### 4.3.1.3 Distribution of bussstops over Singapore
...
#### 4.3.1.4 Append the busstops data information to bus_hex

```{r}
# Perform the spatial join
bus_hex <- st_intersection(busstop, bus_count) %>%
...
Based on the summary, the median flow volume is 37 and the highest is 132, 650.

We shall look at the quantile distribution to determine the appropriate cut-off value.

```{r}
flowLine$EVENING_PEAK %>% quantile(probs = c(0.95, 0.99))
```
...
Next, a constant distance value of 370m is added into intra-zones distance.
...
Next, plot all the leisure activities on the hexagonal grid
...
The total dwelling units field in the hdb.csv file can be a population proxy in that area.
...
tm_shape(honeycomb_count) +
  ...
  tm_layout(main.title = "HDB Population density",
            main.title.position = "center",
            main.title.size = 1.0,
            legend.height = 0.35, 
            legend.width = 0.35,
...
flow_data2 <- flow_data2 %>%
  ...
  select(-c( n_colli_busstop, total_dwelling_units, area_bus_grid,biz_count,finserv_count,sch_count))
```
...
### 6.5.4 Unconstrained Spatial Interaction Model
...
Next, we will compute the R-squared of the unconstrained uncflow by using the code chunk below.
...
### 6.5.5 Origin- (Production-) Constrained Spatial Interaction Model
...
### 6.5.6 Destination Constrained Spatial Interaction Model
...
### 6.5.7 Doubly Constrained Spatial Interaction Model
...
### 6.5.9 Visualising fitted values
