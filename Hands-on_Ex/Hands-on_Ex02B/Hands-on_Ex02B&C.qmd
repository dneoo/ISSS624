---
title: "Hands-on Exercise 2B & C - Global and Local Measures of Spatial Autocorrelation"
author: "Dabbie Neo"
...
In spatial policy, one of the main development objective of the local government and planners is to ensure equal distribution of development in the province. In this study, we will apply appropriate spatial statistical methods to discover if development are evenly distributed geographically. If the answer is **No**, our next question will be "is there sign of spatial clustering?". And, if the answer is yes, then our next question will be "where are these clusters?"
...
Before we can compute the global spatial autocorrelation statistics, we first need to construct a spatial weights of the study area. The spatial weights is used to define the neighbourhood relationships between the geographical units (i.e. county) in the study area.

In the code chunk below, [*poly2nb()*](https://r-spatial.github.io/spdep/reference/poly2nb.html) of **spdep** package is used to compute contiguity weight matrices for the study area. This function builds a neighbours list based on regions with contiguous boundaries. If we look at the documentation, we will see that the "queen" argument can take TRUE (default) or FALSE as options. 
...
::: callout-note
The Moran's I statistic is a value that ranges from -1 (indicating perfect dispersion) to +1 (perfect correlation). A value near 0 indicates a random spatial pattern. In this case, the Moran's I value is approximately 0.30 which suggests a moderate positive spatial autocorrelation. 

Given the p-value of 1.095e-06, which is much smaller than 0.05, there is a strong evidence to reject the null hypothesis of no spatial autocorrelation. Thus, suggesting there is a statistically significant positive spatial autocorrelation.This means that areas with similar values of GDPPC are more likely to be located near each other than would be expected if the locations were randomly distributed. 
:::

#### 4.3.1.1 Computing Monte Carlo Moran's I
...
Given that the p-value is less than 0.05, we would reject the alternative hypothesis, which suggests that there is a greater level of spatial autocorrelation than would be expected by chance. This reinforces the previous conclusion that the GDPPC is not randomly distributed across the studied area, but rather exhibits significant positive spatial autocorrelation.
:::

#### 4.3.1.2 Visualising Monte Carlo Moran's I
...
The code chunk below uses ggplot2 package to plot the values, 

```{r}
library(ggplot2)

data_gg <- data.frame(simulated_morans_i = bperm$res)

# Create the histogram using ggplot2
ggplot(data_gg, aes(x = simulated_morans_i)) +
  geom_histogram(binwidth = 0.025, fill = 'grey', color = 'black') + # Adjust binwidth as needed
  geom_vline(aes(xintercept = 0), color = "red") +
  labs(title = "Histogram of Simulated Geary c", 
       x = "Simulated Geary c", 
       y = "Frequency") +
  theme_minimal()
```
...
The Geary C statistic ranges from 0 to 2, where:

A value of 1 suggests no spatial autocorrelation.
A value less than 1 indicates positive spatial autocorrelation (similarity between neighboring locations).
A value greater than 1 indicates negative spatial autocorrelation (dissimilarity between neighboring locations).
:::

Given that the Geary C statistic (0.6907223) is less than 1, suggests positive spatial autocorrelation. This means that neighbouring areas tend to more similar in terms of GDPCC. 

The p-values is also very small, less than 0.05, thus we would reject the null hypothesis of no spatial autocorrelation and conclude that there is statistically significant evidence of positive spatial autocorrelation in the data.


#### 4.3.1.1 Computing Monte Carlo Geary's C
...
#### 4.3.1.2 Visualising the Monte Carlo Geary's C
...
Spatial correlograms is a great way to examine patterns of spatial autocorrelation in your data or model residuals. They show how correlated the pairs of spatial observations are when you increase the distance (lag) between them - they are plots of some index of autocorrelation (Moran's I or Geary's c) against distance. Although correlograms are not as fundamental as variograms (a keystone concept of geostatistics), they are very useful as an exploratory and descriptive tool. For this purpose, they actually provide richer information than variograms.
...
By plotting the output, it might not allow us to provide a complete interpretation. This is because not all autocorrelation values are statistically significant. Hence, it is important for us to examine the full analysis report by printing out the analysis results as in the code chunk below.
...
#### 4.4.1.1 Compute Geary's C correlogram and plot
