...
In this hands-on exercise, [**tmap**](https://cran.r-project.org/web/packages/tmap/) package in R will be our key R package. Besides **tmap** package, four other R packages will be used. They are:
...
To examine the content of `mpsz` , run the code chunk below.
...
Next, create 3 subfolders; geospatial, aspatial and rds.

Place `Master Plan 2014 Subzone Boundary (Web)` into the geospatial folder and *`respopagsex2000to2018`* into the aspatial folder.
...
```{r}
write_rds(mpsz_pop2020, "data/rds/mpszpop2020.rds")  #create an rds folder in data folder
```
...
In the following sub-section, we will use the **tmap functions** to plot these elements.
...
Now, change the palette to *Green*.
...
Instead of creating small multiple choropleth map, we can also use the selection funtion to map spatial objects meeting the selection criterion.
