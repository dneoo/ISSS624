...
For the purpose of this exercise, the following packages will be used:

-   **sf** for importing, integrating, processing and transforming geospatial data.
-   **tidyverse** for importing, integrating, wrangling and visualising data.
-   **tmap** for creating thematic maps.
-   **reshape2**have a more effective way to work with matrix
-   **ggpubr** for creating multiple plot into one

```{r}
pacman::p_load(tmap, sf, sp, DT, reshape2, spatstat, performance, 
...
dist <- spDists(mpsz_sp, 
                longlat = FALSE)     #subzone already has units, SVY 21, thus put longlat=FALSE
```

The code chunk below is used to display the first 10 rows and columns of the matrix.
...
::: callout-Warining
Don't sort the data, it will affect the order.
:::
...
```{r}
#| eval: false
distPair$dist <- ifelse(distPair$dist == 0,   #(175/2 ~ 80, then choose 50)
                        50, distPair$dist)
#any 0 then update to 50
```
...
Pop dataset only has subzone name, but flow data only has subzone code. Thus, the code chunk below is used to bring in the SUBZONE_N and the SUBZONE_C in mpsz and match it to the pop data set to get the subzone code, so that we can append the data to the flow data for further analysis.
...
Note the SZ column in pop data is a general subzone code, it does not refer to origin or destination subzone code in particular.

### Preparing origin attribute

Combine flow data with the pop dataset by the subzone and renane the columns.
...
![](data/unconstrained_spatial_formula.PNG)
...
![](data/origin_constrained_spatialformula.PNG)
...
![](data/destination_constained_spatialformula.PNG)
...
![](data/doubly_constrained_spatialformula.PNG)
